      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-primary uppercase flex items-center gap-3 text-glow">
            ...
            مصفوفة المنصات العالمية
          </h1>
          <p className="text-muted-foreground mt-2 text-sm">
            تغطية النظام الحالية عبر الشبكات الاجتماعية وقواعد البيانات والنقاط الإقليمية.
          </p>
        </div>
        <div className="flex items-center gap-4 bg-secondary/40 px-4 py-2.5 rounded-lg border border-border font-mono text-sm shrink-0">
      ...
      {isLoading ? (
        ...
        <div className="space-y-10">
          {orderedCategories.map((category) => {
            ...
            return (
              <div key={category} className="space-y-4">
                <h2 className="text-base uppercase tracking-widest text-foreground flex items-center gap-2 border-b border-border/40 pb-3 font-mono">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  {items.map((platform, idx) => (
                    <div
                      key={platform.slug}
                      className={`p-3.5 rounded border font-mono flex flex-col justify-between h-20 transition-all duration-200 stagger-item ${
                        platform.active
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span
                          className="text-sm font-bold truncate text-foreground/90 group-hover:text-primary transition-colors"
                          title={platform.name}
                        >
                          {platform.name}
                        </span>
          ...
          {filteredPlatforms.length === 0 && (
            <div className="text-center p-14 text-muted-foreground border border-dashed border-border rounded-lg text-sm">
